...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    """
    mask_x = np.linspace(border, img.shape[1] - border + 1, nPointsX, dtype=np.int32)
    mask_y = np.linspace(border, img.shape[0] - border + 1, nPointsY, dtype=np.int32)
    mask_x, mask_y = np.meshgrid(mask_x, mask_y)

    mask_x = mask_x.flatten()
    mask_y = mask_y.flatten()

    vPoints = np.c_[mask_x, mask_y]
    return vPoints
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    angle = np.arctan2(grad_y, grad_x)  # [h, w]
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                histo = np.histogram(angle[start_y:end_y, start_x:end_x], bins=nBins, range=(-np.pi, np.pi))
                desc.append(histo[0])
...
def bow_histogram(vFeatures, vCenters):
    ...
    dist = np.zeros((num_centers, num_features))
    for i in range(num_centers):
        dist[i] = np.linalg.norm(vFeatures - vCenters[i], axis=1)
    
    histo = np.histogram(np.argmin(dist, axis=0), bins=num_centers)[0]
    return histo
