...
from impl.util import HNormalize, MakeHomogeneous
...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  normalized_kps1 = (inverse_K @ MakeHomogeneous(im1.kps, 1).T).T
  normalized_kps2 = (inverse_K @ MakeHomogeneous(im2.kps, 1).T).T

  # Assemble constraint matrix as equation 2.1
  constraint_matrix = np.zeros((matches.shape[0], 9))
  for i in range(matches.shape[0]):
    kp1 = normalized_kps1[matches[i,0]] #(x', y')
    kp2 = normalized_kps2[matches[i,1]] #(x, y)

    constraint_matrix[i,:] = np.array([
      kp1[0] * kp2[0], #x'x
      kp1[0] * kp2[1], #x'y
      kp1[0],          #x'
      kp1[1] * kp2[0], #y'x
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for reg_name in registered_images:
    ...
    corrs[reg_name] = (reg_corr, num_points, num_points + points.shape[0])

    # Add the correspondences to the image
    all_image_corr = np.concatenate([all_image_corr, im_corr], 0)
    num_points += num_new_points

  # Add the correspondences of the new image
  corrs[image_name] = (all_image_corr, 0, num_points)
